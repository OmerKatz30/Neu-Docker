...
def test_reverse_api_success(client, input_str, expected):
    # The fix is to pass the query string as a dictionary to handle special characters correctly.
    response = client.get("/reverse", query_string={'in': input_str})
...
def test_reverse_empty_input(client):
    response = client.get("/reverse", query_string={'in': ""})
    assert response.status_code == 400
    data = response.json
    assert "error" in data
...
def test_restore_after_reverse(client):
    ...
    client.get("/reverse", query_string={'in': input_str})
